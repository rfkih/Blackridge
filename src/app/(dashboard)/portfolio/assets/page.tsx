...
function ExecuteConfirmDialog({
  ...
  return (
    <div
      ...
      onClick={(e) => {
        // Backdrop dismiss: only when the click landed on the backdrop itself,
        // not inside the dialog (replaces the stopPropagation pattern so the
        // dialog div carries no event handlers).
    ...
    >
      ...
      >
        ...
        <label htmlFor="execute-confirm-input" className="block text-[11px] text-text-muted">
          Type <span className="font-mono font-bold text-text-primary">EXECUTE</span> to confirm:
        </label>
        <input
          id="execute-confirm-input"
          type="text"
          ...
          // eslint-disable-next-line jsx-a11y/no-autofocus -- modal focus pattern: focus must move into the dialog on open, and the confirm input is its primary control
          autoFocus
        />
