...
export interface VboParamFormProps {
  ...
  strategyCode?: string;
  initialValues: Partial<VboParams>;
  defaultValues: VboParams;
  onChange?: (key: keyof VboParams, value: unknown) => void;
...
export function VboParamsForm({
  ...
  const [values, setValues] = useState<VboParams>(() => mergeInitial(defaultValues, initialValues));
  const [savedValues, setSavedValues] = useState<VboParams>(() =>
    mergeInitial(defaultValues, initialValues),
  );
  const [confirmSaveOpen, setConfirmSaveOpen] = useState(false);
  const [confirmResetOpen, setConfirmResetOpen] = useState(false);
  const toast = useToast();
  const saveMutation = useSaveVboParams(accountStrategyId);
  const resetMutation = useResetVboParams(accountStrategyId);
  ...
  const handleFieldChange = useCallback(
    (key: keyof VboParams, value: unknown) => {
  ...
  const overrideCountByKey = useMemo(() => {
    ...
    VBO_SECTIONS.forEach(({ title, keys }) => {
  ...
  const resetAll = useCallback(() => {
    ...
    if (mode === 'backtest') {
      (Object.keys(defaultValues) as Array<keyof VboParams>).forEach((k) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3">
        {VBO_SECTIONS.map((section, idx) => (
          <ParamSection
            key={section.title}
            title={section.title}
            storageKey={`bh:param-section:vbo:${section.title}`}
            defaultOpen={idx === 0}
            overrideCount={overrideCountByKey[section.title] ?? 0}
          >
            {section.keys.map((key) => {
              const meta = VBO_PARAM_META[key];
              return (
                <ParamField
                  key={key}
                  name={key}
