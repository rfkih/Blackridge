...
export interface LsrParamFormProps {
  ...
  defaultValues: LsrParams;
  onChange?: (key: keyof LsrParams, value: unknown) => void;
  /** Only relevant in backtest mode — allows pushing params to the live account. */
...
export function LsrParamsForm({
  ...
  const handleFieldChange = useCallback(
    (key: keyof LsrParams, value: unknown) => {
  ...
  const overrideCountByKey = useMemo(() => {
    ...
    LSR_SECTIONS.forEach(({ title, keys }) => {
  ...
  const resetAll = useCallback(() => {
    ...
    if (mode === 'backtest') {
      (Object.keys(defaultValues) as Array<keyof LsrParams>).forEach((k) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3">
        {LSR_SECTIONS.map((section, idx) => (
          <ParamSection
            key={section.title}
            title={section.title}
            storageKey={`bh:param-section:lsr:${section.title}`}
            defaultOpen={idx === 0}
            overrideCount={overrideCountByKey[section.title] ?? 0}
          >
            {section.keys.map((key) => {
              const meta = LSR_PARAM_META[key];
              return (
                <ParamField
                  key={key}
                  name={key}
                  ...
                  disabled={mode === 'live' && meta.readOnly}
                />
              );
            })}
          </ParamSection>
