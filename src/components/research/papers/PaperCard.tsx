...
export function PaperCard({ paper }: PaperCardProps) {
  ...
  return (
    ...
    >
      ...
      {paper.abstract && (
        <p className="line-clamp-3 text-[14px] leading-relaxed text-text-secondary">
          {paper.abstract}
        </p>
      )}
      ...
      <div className="mt-auto flex items-center justify-between gap-2 pt-1">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-[13px] font-semibold text-[var(--accent-primary)] opacity-0 transition-opacity group-hover:opacity-100">
            Read →
          </span>
        </div>
      </div>
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-sm border border-bd-subtle bg-bg-base px-1.5 py-0.5 font-mono text-[12px] text-text-secondary">
      {children}
    </span>
  );
}
