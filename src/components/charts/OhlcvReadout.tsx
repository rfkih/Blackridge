...
function fmt(n: number | null, _sym: string): string {
  if (n == null) return '—';
...
const SEP = <span className="mx-2 select-none text-[var(--border-strong)]">|</span>;
...
export function OhlcvReadout({
  ...
  const change =
    close != null && previousClose != null ? ((close - previousClose) / previousClose) * 100 : null;
  const isProfit = (change ?? 0) >= 0;

  return (
    <div className="scrollbar-none flex items-center gap-0 overflow-x-auto px-4 py-1.5 font-mono text-xs text-[var(--text-primary)]">
