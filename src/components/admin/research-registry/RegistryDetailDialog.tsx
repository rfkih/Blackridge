...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-bd-subtle bg-bg-base px-2.5 py-1.5">
      ...
      <div className="mt-0.5 font-mono tabular-nums text-[14px] text-text-primary">{value}</div>
    </div>
  );
}
...
function EvidenceRow({ label, value }: { label: string; value: string | null }) {
  ...
  return (
    <div className="flex items-baseline gap-2">
      <span className="w-28 shrink-0 text-[12px] uppercase tracking-widest text-text-muted">
        {label}
      </span>
      <span className="break-all font-mono text-[13px] text-text-secondary">{value}</span>
    </div>
  );
}
          ...
          <>
            <DialogHeader>
              <DialogTitle className="flex flex-wrap items-center gap-2 text-text-primary">
                <span className="font-mono text-[14px] text-text-muted">#{entry.rank ?? '—'}</span>
                {entry.displayName}
              </DialogTitle>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {sv && (
                <span
                  className="rounded-sm px-1.5 py-px font-mono text-[12px] uppercase tracking-widest"
            ...
            {entry.divergence.flag && (
              <div
                className="flex items-start gap-2 rounded-md px-3 py-2 text-[14px]"
            ...
            <div>
              <div className="mb-1 text-[12px] uppercase tracking-widest text-text-muted">
                Live metrics{' '}
