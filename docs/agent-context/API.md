...
| Leaderboard | `/api/v1/leaderboard/top-strategies` `/deploy` | GET/POST | Any-user. `GET /top-strategies?limit=` ranks the V102-approved set by risk-adjusted score (server clamps limit 1–50). `POST /deploy` (`{accountId, symbol, strategyCode, capitalAllocationPct, activate?}`) one-click adopts a ranked strategy onto the caller's account, returns the created `account_strategy` row. `activate` (default/`true`) lands LIVE, deactivating any existing active sibling for the `(account, strategy, symbol, interval)` tuple; `false` lands STOPPED so the sibling keeps running (the dialog's conflict prompt drives this). Reads only approved evidence. |
